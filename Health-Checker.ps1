...
begin {
    $repoRawBaseUrl = "https://raw.githubusercontent.com/microsoft/CSS-Exchange/main"
    $requiredScripts = @(
        @{ LocalPath = (Join-Path $PSScriptRoot "Helpers\Get-ErrorsThatOccurred.ps1"); RepoPath = "Diagnostics/HealthChecker/Helpers/Get-ErrorsThatOccurred.ps1" }
        @{ LocalPath = (Join-Path $PSScriptRoot "Helpers\Get-ExportedHealthCheckerFiles.ps1"); RepoPath = "Diagnostics/HealthChecker/Helpers/Get-ExportedHealthCheckerFiles.ps1" }
        @{ LocalPath = (Join-Path $PSScriptRoot "Helpers\Invoke-ConfirmExchangeShell.ps1"); RepoPath = "Diagnostics/HealthChecker/Helpers/Invoke-ConfirmExchangeShell.ps1" }
        @{ LocalPath = (Join-Path $PSScriptRoot "Helpers\Invoke-SetOutputInstanceLocation.ps1"); RepoPath = "Diagnostics/HealthChecker/Helpers/Invoke-SetOutputInstanceLocation.ps1" }
        @{ LocalPath = (Join-Path $PSScriptRoot "Writers\Write-ResultsToScreen.ps1"); RepoPath = "Diagnostics/HealthChecker/Writers/Write-ResultsToScreen.ps1" }
        @{ LocalPath = (Join-Path $PSScriptRoot "Writers\Write-Functions.ps1"); RepoPath = "Diagnostics/HealthChecker/Writers/Write-Functions.ps1" }
        @{ LocalPath = (Join-Path $PSScriptRoot "Features\Get-HtmlServerReport.ps1"); RepoPath = "Diagnostics/HealthChecker/Features/Get-HtmlServerReport.ps1" }
        @{ LocalPath = (Join-Path $PSScriptRoot "Features\Get-LoadBalancingReport.ps1"); RepoPath = "Diagnostics/HealthChecker/Features/Get-LoadBalancingReport.ps1" }
        @{ LocalPath = (Join-Path $PSScriptRoot "Features\Get-ExchangeDcCoreRatio.ps1"); RepoPath = "Diagnostics/HealthChecker/Features/Get-ExchangeDcCoreRatio.ps1" }
        @{ LocalPath = (Join-Path $PSScriptRoot "Features\Get-MailboxDatabaseAndMailboxStatistics.ps1"); RepoPath = "Diagnostics/HealthChecker/Features/Get-MailboxDatabaseAndMailboxStatistics.ps1" }
        @{ LocalPath = (Join-Path $PSScriptRoot "Features\Invoke-HealthCheckerMainReport.ps1"); RepoPath = "Diagnostics/HealthChecker/Features/Invoke-HealthCheckerMainReport.ps1" }
        @{ LocalPath = (Join-Path $PSScriptRoot "Features\Invoke-VulnerabilityReport.ps1"); RepoPath = "Diagnostics/HealthChecker/Features/Invoke-VulnerabilityReport.ps1" }
        @{ LocalPath = (Join-Path $PSScriptRoot "..\..\Shared\Confirm-Administrator.ps1"); RepoPath = "Shared/Confirm-Administrator.ps1" }
        @{ LocalPath = (Join-Path $PSScriptRoot "..\..\Shared\ErrorMonitorFunctions.ps1"); RepoPath = "Shared/ErrorMonitorFunctions.ps1" }
        @{ LocalPath = (Join-Path $PSScriptRoot "..\..\Shared\Get-PSSessionDetails.ps1"); RepoPath = "Shared/Get-PSSessionDetails.ps1" }
        @{ LocalPath = (Join-Path $PSScriptRoot "..\..\Shared\LoggerFunctions.ps1"); RepoPath = "Shared/LoggerFunctions.ps1" }
        @{ LocalPath = (Join-Path $PSScriptRoot "..\..\Shared\OutputOverrides\Write-Error.ps1"); RepoPath = "Shared/OutputOverrides/Write-Error.ps1" }
        @{ LocalPath = (Join-Path $PSScriptRoot "..\..\Shared\OutputOverrides\Write-Host.ps1"); RepoPath = "Shared/OutputOverrides/Write-Host.ps1" }
        @{ LocalPath = (Join-Path $PSScriptRoot "..\..\Shared\OutputOverrides\Write-Verbose.ps1"); RepoPath = "Shared/OutputOverrides/Write-Verbose.ps1" }
        @{ LocalPath = (Join-Path $PSScriptRoot "..\..\Shared\OutputOverrides\Write-Warning.ps1"); RepoPath = "Shared/OutputOverrides/Write-Warning.ps1" }
        @{ LocalPath = (Join-Path $PSScriptRoot "..\..\Shared\ScriptUpdateFunctions\Test-ScriptVersion.ps1"); RepoPath = "Shared/ScriptUpdateFunctions/Test-ScriptVersion.ps1" }
    )

    foreach ($requiredScript in $requiredScripts) {
        if (-not (Test-Path -LiteralPath $requiredScript.LocalPath)) {
            $dependencyDirectory = Split-Path -Path $requiredScript.LocalPath -Parent
            if (-not (Test-Path -LiteralPath $dependencyDirectory)) {
                New-Item -Path $dependencyDirectory -ItemType Directory -Force | Out-Null
            }

            $downloadUrl = "$repoRawBaseUrl/$($requiredScript.RepoPath)"
            Write-Host "Missing dependency detected. Downloading from GitHub: $($requiredScript.RepoPath)"

            try {
                Invoke-WebRequest -Uri $downloadUrl -UseBasicParsing -OutFile $requiredScript.LocalPath -ErrorAction Stop
            } catch {
                throw "Failed to download required dependency '$($requiredScript.RepoPath)' from '$downloadUrl'. $($_.Exception.Message)"
            }
        }

        . $requiredScript.LocalPath
    }
